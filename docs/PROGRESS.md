...
- ✅ **已完成**
  ...
  - **金額計算修正 + 卡片 UI（2026-06-19 ~ 2026-06-20）：**
    - `order_items` 加 `price` 欄位（migration `0009_add_price_to_order_items.sql`）— ✅ 已套用至 Supabase
    ...
    - `e2e/menus.test.mjs`、`e2e/stores.test.mjs`、`e2e/order-summary.test.mjs` 同步更新為卡片選取器（`[data-menu-store]`、`[data-store-name]`），所有 E2E 測試通過
