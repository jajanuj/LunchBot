...
export default function StoreEditForm({ store }: { store: StoreTemplate }) {
  const [state, formAction, pending] = useActionState(updateStoreAction, undefined);
  const [storeName, setStoreName] = useState(store.storeName);
  const [items, setItems] = useState<ItemRow[]>(
    store.items.length > 0
      ? store.items.map((i) => ({ key: makeKey(), itemName: i.itemName, price: String(i.price) }))
