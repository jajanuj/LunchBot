...
export default function OrderSummarySection({
  ...
  const totalAmt = storeSummary.reduce((s, r) => s + r.totalAmount, 0);

  function exportCsv() {
    const rows: string[] = ["員工姓名,品項,數量,單價,小計"];
    for (const bill of billingRows) {
      for (const item of bill.items) {
        rows.push(
          [bill.employeeName, item.itemName, item.quantity, item.price, item.price * item.quantity]
            .map((v) => `"${v}"`)
            .join(",")
        );
      }
    }
    rows.push(`"合計","","${totalQty}","","${totalAmt}"`);
    ...
    <div className="flex flex-col gap-4 mb-4">
      ...
      <details id="store-order-summary" className="border rounded p-4">
        <summary className="cursor-pointer font-medium">
          店家叫貨清單（共 {totalQty} 份，合計 ${totalAmt}）
        </summary>
        <table className="w-full border-collapse text-left mt-3">
          ...
          <tbody>
            ...
            <tr className="font-semibold">
              ...
              <td className="py-2 pr-4">${totalAmt}</td>
            </tr>
          </tbody>
        </table>
            ...
            {billingRows.map((bill) =>
              bill.items.map((item, idx) => (
                <tr key={`${bill.employeeId}-${item.menuItemId}`} className="border-b">
                  {idx === 0 && (
                    <td className="py-2 pr-4 font-medium" rowSpan={bill.items.length}>
                      {bill.employeeName}
                    </td>
                  )}
